...
| `referralJudgement` 的 `recentTurns` / `matchedText` | **是**（2026-09-17 补上） | 与 `proxyDeepSeek` 同一 `stripPII`，逐轮剥离后再拼接判断 prompt |
...
1. ~~`referralJudgement` 补 stripPII~~ 已完成（2026-09-17）。
